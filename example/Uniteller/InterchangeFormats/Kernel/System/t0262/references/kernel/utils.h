  ...
  namespace Framework
  {
    ...
    {   
      ...
      inline bool StartsWith(const std::string& a, const std::string& b)
      {
        ...
        template <class T>
        bool FromHex(const std::string & s, T & value)
        {
        ...
        {
#ifndef __GNUG__
          return (arg.size() > 0) && (arg.front() == left) && (arg.back() == right);
#else
#if __GNUC__ < 5
          return (arg.size() > 0) && (arg[0]==left) && (arg[arg.size()-1]==right);
#else
          return (arg.size() > 0) && (arg.front()==left) && (arg.back()==right);
#endif
#endif
        ...
        /// <summary>
        /// Дополняет строку слева переданным символом
        /// </summary>
        inline std::string & PadLeft(std::string & s, char c, size_t len)
        {
          const size_t v = s.size();
          if (v<len)
            s.insert(s.begin(), len - v, c);
          return s;
        }
