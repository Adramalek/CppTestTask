    ...
    namespace Kernel
    {
      namespace Protocol
      {
        ...
        BEGIN_PROTOCOL(0, 1)
          ...
          /// <summary> 
          /// Ошибки средств упаковки и разбора по спецификации формата данных для протокола
          /// </summary>
          enum class TransformationErrors
          {
            ArgumentOutOfRange = 1, // - Индекс аргумента превышает допустимое число аргументов сообщения данного типа
            EmptyPackage = 5, // - Пустой пакет для разбора
            InvalidLength = 4, // - Неверная длина
            InvalidLrc = 2, // - Неверная контрольная сумма Lrc
            InvalidParameterFormat = 7, // - Некорректный формат параметра
            ParsingError = 6, // - Ошибка разбора
            UnrecognizedKeyGroup = 3 // - Неопознанная ключевая группа
          };
          ...
          /// <summary>
          /// Валидатор корректности значения для перечислимого типа TransformationErrors
          /// </summary>
          inline bool IsValidTransformationErrorsValue(const int value)
          {
            return ((value >= 1)&&(value < 8));
          }
          
          /// <summary>
          /// Следует использовать для преобразования числовых значений к перечислимому типу TransformationErrors
          /// </summary>
          inline TransformationErrors ConvertToTransformationErrors(const int value)
          {
            switch (value)
            {
              case 1:
                return TransformationErrors::ArgumentOutOfRange;
              case 5:
                return TransformationErrors::EmptyPackage;
              case 4:
                return TransformationErrors::InvalidLength;
              case 2:
                return TransformationErrors::InvalidLrc;
              case 7:
                return TransformationErrors::InvalidParameterFormat;
              case 6:
                return TransformationErrors::ParsingError;
              case 3:
                return TransformationErrors::UnrecognizedKeyGroup;
              default: throw KernelException("Невозможно преобразовать числовое значение %d к типу TransformationErrors", value);
              }
          }
